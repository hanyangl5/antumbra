...
gal_error_code vk_create_instance(gal_desc *gal_desc, gal_context *context) {
    ...
    if (required_layer_count != 0) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: unsupported instance layer");
        }
        return gal_error_code::ERR;
    }
    ...
    if (required_extension_count != 0) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: unsupported instance extension");
        }
        return gal_error_code::ERR;
    }
    ...
    if (res == VK_SUCCESS && vk_ctx->instance != VK_NULL_HANDLE) {
        ...
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to create vk instance, error_code is {}",
                      ant::enum_refl::enum_name<VkResult>(res));
        }
        return gal_error_code::ERR;
    }
}
...
gal_error_code vk_create_device(gal_desc *gal_desc, gal_context *context) {
    ...
    if (physical_device_count == 0) {
        if (ant::gal::b_enable_validation_layer) {
            LOG_ERROR("[gal] no available gpu");
        }
        return gal_error_code::ERR;
    }
    ...
    if (result != gal_error_code::SUC || vk_ctx->active_gpu == VK_NULL_HANDLE) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: no available gpu");
        }
        return gal_error_code::ERR;
    ...
    if (res != VK_SUCCESS || vk_ctx->device == VK_NULL_HANDLE) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to create vk device, error_code is {}",
                      ant::enum_refl::enum_name<VkResult>(res));
        }
        return gal_error_code::ERR;
    }
...
gal_error_code vk_create_memory_allocator(gal_context *context) {
    ...
    if (result == VK_SUCCESS && vk_ctx->vma_allocator != VK_NULL_HANDLE) {
        ...
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to create vma, error_code is {}", ant::enum_refl::enum_name<VkResult>(result));
        }
        return gal_error_code::ERR;
    }
}
...
gal_error_code vk_create_buffer(gal_context context, gal_buffer_desc *desc, gal_buffer *buffer) {
    ...
    if (!vk_buf) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to create allocate buffer object");
        }
        return gal_error_code::ERR;
    }
    *buffer = reinterpret_cast<gal_buffer>(vk_buf);
    ...
    if (result != VK_SUCCESS || vk_buf->m_buffer == VK_NULL_HANDLE || vk_buf->m_allocation == VK_NULL_HANDLE) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to create vk buffer, error_code is {}", ant::enum_refl::enum_name<VkResult>(result));
        }
        return gal_error_code::ERR;
    }
...
gal_error_code vk_create_texture(gal_context context, gal_texture_desc *desc, gal_texture *texture) {
    ...
    *texture = reinterpret_cast<gal_texture>(vk_tex);
    if (!*texture) {
        if (b_enable_validation_layer) {
            LOG_ERROR("[gal]: failed to allocate texture object");
        }
        return gal_error_code::ERR;
    }
    vk_tex->m_desc = *desc;
    ...
    if (ant::gal::b_enable_validation_layer) {
        if (image_create_info.imageType == VK_IMAGE_TYPE_MAX_ENUM) {
            LOG_ERROR("[gal]: invalid image dimension");
        }
        if (image_create_info.format == VK_FORMAT_UNDEFINED) {
            
        }

        //LOG_ERROR("invalid descriptor type: {}", static_cast<u32>(types));
    }
    // cubemap flags
    if (b_is_cubemap) {
