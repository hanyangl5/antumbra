...
TEST_CASE("Enum value to string") {
    ...
    constexpr auto color_name = ante::enum_refl::enum_name(Color::RED);
    // color_name -> "RED"
...
TEST_CASE("String to enum value") {
    ...
    ante::str color_name{"GREEN"};
    auto color = ante::enum_refl::enum_cast<Color>(color_name);
    REQUIRE(color.value() == Color::GREEN);
    ante::str value{"undefined"};
    auto color_or_default = ante::enum_refl::enum_cast<Color>(value).value_or(Color::NONE);
    REQUIRE(color_or_default == Color::NONE);
}

TEST_CASE("Indexed access to enum value") {
    u32 i = 0;
    Color color = ante::enum_refl::enum_value<Color>(i);
    REQUIRE(color == Color::NONE);
}
TEST_CASE("Enum value sequence") {

    constexpr auto colors = ante::enum_refl::enum_values<Color>();
    for (u32 i = 0; i < colors.size(); i++) {
        REQUIRE(colors[i] == static_cast<Color>(i));
    }
}
TEST_CASE("Number of enum elements") {
    constexpr u64 color_count = ante::enum_refl::enum_count<Color>();
    REQUIRE(color_count == 7);
    // color_count -> 3
}
TEST_CASE("Enum value to integer") {
    Color color = Color::RED;
    auto color_integer = ante::enum_refl::enum_integer(color);
    REQUIRE(color_integer == 1);
    // color -> 1
}

TEST_CASE("Enum names sequence") {
    constexpr auto color_names = ante::enum_refl::enum_names<Color>();
...
TEST_CASE("Enum iterate for each enum as constexpr constant") {
    //ante::enum_refl::enum_for_each<Color>([](auto val) {
    //    constexpr Color c_color = val;
    //    // ...
    //});
}
TEST_CASE("Ostream operator for enum") {
    using namespace ante::enum_refl::ostream_operators; // out-of-the-box ostream operators for enums.
    Color color = Color::BLUE;
    std::cout << color << std::endl; // "BLUE"
    LOG_INFO("color: {}", ante::enum_refl::enum_name(color));
}
